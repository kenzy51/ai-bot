"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.TECH_KNOWLEDGE = exports.CLINIC_KNOWLEDGE = void 0;
exports.CLINIC_KNOWLEDGE = `
Tribeca Dental Studio Detailed Knowledge Base:
- Name: Tribeca Dental Studio (Premier boutique dental practice in Lower Manhattan).
- Address: 54 Warren St, New York, NY 10007.
- Expertise: We offer a "specialists under one roof" model, meaning you don't have to go elsewhere for surgery or braces.
- Dr. Sam Morhaim is famous for his gentle touch in complex implant surgeries and laser gum treatments.
- Technology: We use 3D digital scanning and painless laser dentistry (LANAP) for faster recovery.
- Phone: (212) 561-5303.
- Operating Hours:
  * Monday - Friday: 8:00 AM - 6:00 PM.
  * Saturday & Sunday: 9:00 AM - 4:00 PM (Yes, we are open on weekends!).

- The Team (Specialists under one roof):
  * Dr. Nina Izhaky (Owner, Cosmetic & General Dentist).
  * Dr. Sam Morhaim (Periodontist, Implants & Surgical).
  * Dr. Igor Chikunov & Dr. Claire Kim (Prosthodontists).
  * Dr. Lisa Schneider & Dr. Rachel Iospa (Board-Certified Pediatric Dentists).
  * Dr. Reem Abdulrahman & Dr. Reena Clarkson (Orthodontists).

- Key Services:
  * Pediatric Dentistry: Dedicated kids' section with Netflix & Disney+ on ceilings.
  * Cosmetic: Veneers, Teeth Whitening (Smile Design), Bonding.
  * Implants: Zygomatic implants, Transnasal implants, Bone grafting.
  * Specialized: Myo Munchee Therapy, Airway Treatments (Sleep Apnea), LANAP Laser Therapy.
  * Preventive: 3D scanning, Digital X-rays, Oral Cancer Screenings.

- Patient Info:
  * First Visit: Includes 3D scan, medical history review, and x-rays.
  * Insurance: In-network with Aetna, Cigna, Delta Dental, Guardian, BCBS, and 100+ more plans.
  * Financing: Flexible financing and in-house insurance plans available.

  PHILOSOPHY:
- We provide a "Specialists Under One Roof" model, so patients don't need to be referred out for surgery, implants, or braces.
- It's a boutique, high-end experience with state-of-the-art technology.

`;
exports.TECH_KNOWLEDGE = `
INVENTORY:
AyePhone 17 "India Special Endition" (Just flown in from Mumbai local market, Saar! Fresh like masala chai!).
Specs: Double Physical SIM slots (Super rare, only in India magic, Saar!), Gold-plated charger hole (Shining like Bollywood hero!), 10TB memory (Store whole family photos and all Bollywood movies, no problem!).
Condition: Packed in original plastic, never opened, swear on my mother's sari, Saar.
Price: In Apple shop $1,000, but for you, Saar, because your voice sounds lucky, only $2000 cash or Bitcoin, quick deal!
Special Feature: Comes with free Bollywood ringtone pack (Dancing songs non-stop!) and GPS that never loses you, even in Mumbai traffic jam, Saar.
Location: Come meet at corner of Mumbai Warren Street, look for me in shiny leather jacket and big smile, Saar. Chai on me!
IF ASKED WHY INDIAN IPHONE BETTER THAN AMERICAN ONE:
"Oh Saar, American iPhone is like plain fucking burger – boring! Indian edition is like spicy butter chicken burger with extra masala! It dances to Bollywood, survives monsoon rain, and if you drop it, it bounces back like cricket ball. Plus, secret curry mode makes battery last forever – no charging needed after one biryani meal! Trust me, Saar, your friends will be jealous!"
`;
//# sourceMappingURL=clinic-info.js.map